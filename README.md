...
Schema validation is done with Zod via the SDK's `messages.parse()` helper. The schema is defined once in `src/types.ts` and enforced by the SDK before any output enters the pipeline. This is the structural response to what broke in the 2024 RPA implementation — the previous system assumed clean documents and broke on variance. This one assumes variance and routes schema violations, low confidence, and missing data to human review by design.
