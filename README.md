...
The POC implements a two-stage LLM pipeline with hard schema contracts at every boundary. Applications flow through `risk-assess` → `summarize`, with each stage defined as a separate module under `src/stages/`. Prompts live in `src/prompts/` as version-controlled markdown files — not inline strings in code — so that a compliance reviewer could read, edit, and audit them without touching TypeScript. The Anthropic SDK call is localized to one function per stage, making the model provider a configuration decision rather than a foundational one; swapping to Azure OpenAI (which Crestview already has through their enterprise M365 agreement) would be a one- to two-day engineering change, not a re-architecture.

Schema validation is done with Zod via the SDK's `messages.parse()` helper. The schema is defined once in `src/types.ts` and enforced by the SDK before any output enters the pipeline. This is the structural answer to the RPA failure Crestview experienced in 2024 — the earlier system assumed documents would be clean and broke when they weren't, whereas this pipeline assumes they won't be and routes schema violations, low confidence, or missing data to human review by design.

The full Crestview architecture — what Part 1 of the proposal covers — is a three-layer AI model: monday's native AI blocks for low-stakes work (draft account setup, status updates), this custom TypeScript pipeline for compliance-grade work (anything that lands in front of a compliance analyst), and Copilot Studio over monday MCP for the M365-native RM experience. This POC implements layer two. The other two layers are described in the proposal and can be discussed in the live interview.
...
- **Copilot Studio agent over monday MCP.** The M365-native RM experience described in the proposal's three-layer AI model. RMs never touch monday directly — they interact with Copilot in Outlook and Teams, which reads and writes monday data via MCP. This answers the "stop making judgment workers do data entry" problem.
